...
```{r}
ggplot() +
  layer_spatial(data=commute, aes(fill=wfhE / total_modeE * 100)) +
  scale_fill_binned(palette="Blues", n.breaks=5)
```
...
# I am storing the proportion in a column so I can use it both for coloring and in the quantile
# function.
commute = commute |>
  mutate(proportion_commute_60min = (ttime_60_89E + ttime_90plusE) / total_ttimeE)

ggplot() +
  layer_spatial(data=commute, aes(fill=proportion_commute_60min)) +
  scale_fill_binned(
    breaks=round(unname(quantile(commute$proportion_commute_60min, c(0.2, 0.4, 0.6, 0.8))), digits=2),
    limits=range(commute$proportion_commute_60min)
  ) +
...
age = age |>
  filter(total_popE > 0) |> # avoid dividing by zero
  mutate(pct18_24 = (
...
ggplot() +
  layer_spatial(data=age, aes(fill=pct18_24)) +
  scale_fill_binned(
    breaks=unname(quantile(age$pct18_24, c(0.2, 0.4, 0.6, 0.8))),
    limits=range(age$pct18_24)
  ) +
