...
```{r}
ggplot() +
  layer_spatial(data=census, aes(fill=race_black_african_american_alone)) +
    theme_minimal() +
...
```{r}
ggplot() +
  layer_spatial(data=census, aes(fill=pct_african_american)) +
    theme_minimal() +
    # and this will remove the coordinates from the axes
    theme(
      axis.text = element_blank(),
...
```{r}
ggplot() +
  layer_spatial(data=census) +
  layer_spatial(data=libraries, color="red")
```
...
ggplot() +
  layer_spatial(data=census) +
  layer_spatial(data=libraries_buffer, color="blue") +
  layer_spatial(data=libraries, color="red") +
  theme_minimal() +
  # and this will remove the coordinates from the axes
  theme(
...
```{r}
ggplot() +
  layer_spatial(data=census, aes(fill=library_zone)) +
  layer_spatial(data=libraries, color="red") +
  annotation_scale() +
  theme_minimal() +
  # and this will remove the coordinates from the axes
...
## Exercise: add a basemap

Map the block groups transparent so you can see it.

```{r}
ggplot() +
  annotation_map_tile(zoomin=1) +
  layer_spatial(data=census, aes(fill=library_zone), alpha=0.25) +
  layer_spatial(data=libraries, color="red") +
  annotation_scale() +
  theme_minimal() +
  # and this will remove the coordinates from the axes
  ...
  annotate("label", Inf, -Inf, label="Map data © OpenStreetMap contributors", fill="white", hjust=1.01, vjust=-0.1)

```

We could also visualize this with Leaflet. We have to specify the aesthetics slightly differently, defining a palette and then using it in the map.

```{r}
# use colorFactor for a factor (categorical) variable
# other options are colorBin for a binned scale, see ?colorBin for information
palette = colorFactor("RdBu", domain=census$library_zone)

leaflet() |>
  addTiles() |>
  addPolygons(data=st_transform(census, 4326), fill=TRUE, fillColor=~palette(library_zone), fillOpacity=0.75, weight=0) |>
  addCircleMarkers(data=st_transform(libraries, 4326))
```
