...
# answer:
ggplot() +
    annotation_spatial(data=parcels) +
...
Map that, adding water_buffer to the map below the water bodies but above the parcels. It will look best if you make it partially transparent by putting `alpha=0.5` in `annotation_spatial`. We keep the crs of shadow_spatial as 4326, because we are specifying latitude/longitude values there.

```{r}
ggplot() +
    annotation_spatial(data=parcels) +
    annotation_spatial(data=water_buffer, color="lightblue", fill="lightblue", alpha=0.5) +
    annotation_spatial(data=waterbodies, color="blue", fill="blue") +
    theme_minimal() +
    theme(
        axis.text = element_blank(),
...
```{r}
ggplot() +
    annotation_spatial(data=parcels, aes(fill=at_risk)) +
    annotation_spatial(data=water_buffer, color="lightblue", fill="lightblue", alpha=0.5) +
    annotation_spatial(data=waterbodies, color="blue", fill="blue") +
    theme_minimal() +
    theme(
...
That map would be more useful with some context. We can use `annotation_map_tile` to add tiles from [OpenStreetMap](https://osm.org). We will also need to add attribution.

```{r}
# answer:
ggplot(parcels) +
    # zoomin=1 just makes the basemap more detailed, which I think looks better on this map. annotation_map_tile
    # automatically guesses a zoom level, but you can adjust it with zoomin.
    annotation_map_tile(zoomin=1) +
    annotation_spatial(data=parcels, aes(fill=at_risk), alpha=0.5) +
    annotation_spatial(data=water_buffer, color="lightblue", fill="lightblue", alpha=0.5) +
    annotation_spatial(data=waterbodies, color="blue", fill="blue") +
    shadow_spatial(st_bbox(c(xmin=-79.0755, xmax=-79.0031, ymin=35.8943, ymax=35.9511), crs=4326)) +
    theme_minimal() +
