...
# words table with the words we want to remove.
...
custom_stop_words = bind_rows(
  ...
  tibble(word=c("california", "agency", "eligible", "application", "eligibility", "program", "grant", "fund",
                "project", "funds", "funding", "funded", "applicant", "applicants", "applications", "applicable",
                "government", "project", "million", "requirement", "requirements",
                "loan"
                ))
)

# Remove the stop words and repeat the LDA
tokens = anti_join(tokens, custom_stop_words, by="word")
tokens = mutate(tokens, lemma=lemmatize_words(word))
wcounts = group_by(tokens, title, agency, lemma) %>% summarize(count=n())
word_mtx = cast_dtm(wcounts, title, lemma, count)
model = LDA(word_mtx, 25, control=list(seed=42))
beta = tidy(model, matrix="beta")
top_10 = group_by(beta, topic) %>% slice_max(beta, n=10)
...
ß# Using stems instead of lemmas
tokens = mutate(tokens, stem=stem_words(word))
wcounts = group_by(tokens, title, agency, stem) %>% summarize(count=n())
word_mtx = cast_dtm(wcounts, title, stem, count)
model = LDA(word_mtx, 25, control=list(seed=42))
beta = tidy(model, matrix="beta")
top_10 = group_by(beta, topic) %>% slice_max(beta, n=10)
ggplot(top_10, aes(y=reorder_within(term, beta, topic), x=beta)) +
